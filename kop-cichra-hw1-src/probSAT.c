...
void printSolution() {
    register int i;
    // Output the variable assignments as requested
    for (i = 1; i <= numVars; i++) {
        if (atom[i] == 1) {
            printf("%d ", i);  // Variable is set to true, print as +vi
        } else {
            printf("-%d ", i);  // Variable is set to false, print as -vi
        }
    }
	printf("0\n");
}
...
static inline void printEndStatistics() {
    // Custom simplified output as requested

    // Number of satisfied clauses
    int numSatisfiedClauses = numClauses - numFalse;
    
    // Print the requested output format
    printf("%lli %lli %d %d\n", flip, maxTries * maxFlips, numSatisfiedClauses, numClauses);
}
...
void parseParameters(int argc, char *argv[]) {
    ...
    if (argc > optind + 1) {
        ...
        // Use a more fine-grained seed combining time(0) and gettimeofday()
        struct timeval tv;
        gettimeofday(&tv, NULL);
        seed = (BIGINT) (tv.tv_sec ^ tv.tv_usec);  // Combine seconds and microseconds for a more unique seed
    }
}
